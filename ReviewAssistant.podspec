...
s.version          = '0.1.0'
s.summary          = 'Use ReviewAssistant to summon StoreKit's review controller without having to worry about pacing the review requests. Keep track of recent crashes and review requests.'
...
s.description      = <<-DESC
Use ReviewAssistant to summon StoreKit's review controller without having to worry about pacing the review requests. Track recent crashes with Crashlytics (recommended), save a crash cooldown date after a crash, the app will not ask for review until that date passes. Only asks for review once per version. Never requests for review more than once a day
DESC
